...
`CopierAccount` is the foundational gRPC protocol layer in **JavaCopier**.
...
x-metarpc-client-sdk: JavaCopier/1.0.0
```

## Error Handling

Standard gRPC status codes are translated into idiomatic Java exceptions:
- `UNAUTHENTICATED (16)`: Invalid user key.
